...
# ── SECTION 2A: SCROLLYTELLING — Nov 2024 + Nov 2024–Feb 2025 ────────────────
import streamlit.components.v1 as components
import json
import base64
import pandas as pd
import numpy as np
from utils.data_processing_serbia import prepare_map_json

# ── Encode media files as base64 so iframe can load them ─────────────────────
...
# ── Load + prepare protest data ───────────────────────────────────────────────
protests_df = pd.read_csv("data/protests_serbia.csv")
...
def make_points(df, special_popups=None):
    """
    Convert protest dataframe to list of point dicts for Leaflet.
    Opacity = frequency of protests at that location in same month (more = darker).
    special_popups: dict keyed by (location_lower, date_str) → popup dict
    """
    ...
    for r in df.itertuples():
        ...
        count = counts.get(r.month_loc, 1)
        opacity = round(0.25 + 0.65 * (count / max_count), 2)

        popup = None
        if special_popups:
            popup = special_popups.get((loc_lower, date_str))

        points.append({
            "lat":      float(r.latitude),
            "lon":      float(r.longitude),
            "student":  bool(r.is_student),
            "location": r.location,
            "date":     date_str,
            "opacity":  opacity,
            "popup":    popup,
...
# ── Build phase data ──────────────────────────────────────────────────────────
map_data = prepare_map_json()

# ACT 1 — November 2024 only, zoom to Novi Sad
map_data["act1"] = make_points(
    protests_df[
...
# ACT 2 — Nov 2024 – Feb 2025, voice note at Novi Sad 28.02.2025
...
# ACT 3 — Mar 2025 – Feb 2026, photo at Inđija 02.11.2025
map_data["act3"] = make_points(
    protests_df[
        (protests_df["event_date"] >= "2025-03-01") &
    ...
    special_popups={
        ("inđija", "2025-11-02"): {
            "type":    "photo",
            "file":    photo_b64,
            "caption": "Protest — Inđija, 2 November 2025",
        }
    }
)

# ACT 4 — March 2026
map_data["act4"] = make_points(
    protests_df[protests_df["event_date"] >= "2026-03-01"]
)

map_data_json = json.dumps(map_data)

# ── Weekly data for line chart ────────────────────────────────────────────────
weekly_df = pd.read_csv("data/weekly_serbia.csv")
weekly_df["week"] = pd.to_datetime(weekly_df["week"])
weekly_json = json.dumps([
    {
        "week":      str(r["week"].date()),
        "n_events":  int(r["n_events"]),
        "n_student": int(r["n_student_events"]),
    }
...
# ── HTML/JS scrollytelling builder ───────────────────────────────────────────
...
#steps {{
  flex:0 0 40%; height:{height}px;
  overflow-y:scroll; padding:0 1.5rem 0 0.5rem;
...
.step {{
  ...
  padding:2rem 0; border-bottom:1px solid #1e1e1e;
...
#period-lbl {{
  ...
  border-radius:3px; padding:6px 12px; z-index:1000;
...
#legend {{
  ...
  z-index:1000; font-size:11px; color:#a89f94; pointer-events:none;
}}
...
.op-row {{
  ...
  border-top:1px solid #2a2a2a; font-size:10px; color:#555;
}}
...
.leaflet-popup-content-wrapper {{
  ...
  color:#e8e0d5 !important; font-family:'Times New Roman',serif !important;
}}
...
.leaflet-tooltip {{
  ...
  font-size:11px; padding:3px 7px; border-radius:2px; box-shadow:none;
...
ACTS.forEach((a, i) => {{
  ...
  d.dataset.idx = i;
  d.innerHTML = `
    <div class="s-period">${{a.period}}</div>
    <h2 class="s-heading">${{a.heading}}</h2>
    <p class="s-body">${{a.body}}</p>
    <p class="s-body">${{a.body2}}</p>
  `;
...
// Novi Sad fixed marker
L.circleMarker([45.2671, 19.8335], {{
  radius: 18, color: '#cc3333', weight: 1.5,
  fillColor: '#cc3333', fillOpacity: 0.15,
}}).addTo(nLayer);
L.circleMarker([45.2671, 19.8335], {{
  radius: 5, color: '#cc3333', weight: 0,
  fillColor: '#cc3333', fillOpacity: 1,
}}).bindPopup('<b>Novi Sad</b><br>Train station canopy collapse<br>1 November 2024')
  .addTo(nLayer);
...
function render(act) {{
  ...
  pts.forEach((p, i) => {{
    // Always render special popup markers even if sampled out
    if (i % step !== 0 && !p.popup) return;

    const color  = p.student ? '#7ab8f5' : '#e07b39';
    const radius = p.popup ? 8 : (p.student ? 4 : 3);
    const weight = p.popup ? 1.5 : 0;

let marker;
if (p.popup) {{
  const emoji = p.popup.type === 'audio' ? '🎙' : '📷';
  const icon = L.divIcon({{
    html: `<div style="font-size:20px;line-height:1;filter:drop-shadow(0 1px 3px rgba(0,0,0,0.8));cursor:pointer;">${{emoji}}</div>`,
    className: '',
    iconAnchor: [10, 10],
  }});
  ...
  marker = L.circleMarker([p.lat, p.lon], {{
    ...
    if (p.popup) {{
      ...
      if (p.popup.type === 'audio') {{
        html += `
          <p class="p-caption">${{p.popup.caption}}</p>
          <audio class="p-audio" controls>
            <source src="${{p.popup.file}}" type="audio/mpeg">
            Your browser does not support audio.
          </audio>`;
      }} else if (p.popup.type === 'photo') {{
        html += `
          <p class="p-caption">${{p.popup.caption}}</p>
          <img class="p-img" src="${{p.popup.file}}" alt="${{p.popup.caption}}">`;
      }}
  ...
  // Show legend icons only when relevant
...
with col_text:
    ...
    <div style="display:flex; flex-direction:column; justify-content:center;
      ...
      <p style="font-size:0.62rem; letter-spacing:0.18em; text-transform:uppercase;
         color:#cc3333; margin-bottom:0.3rem;">The Data</p>
      <p style="font-size:0.65rem; letter-spacing:0.1em; color:#444;
         margin-bottom:0.9rem; text-transform:uppercase;">January 2018 – March 2026</p>
      <h2 style="font-size:1.25rem; font-weight:normal; color:#e8e0d5;
         line-height:1.3; margin-bottom:0.9rem; font-family:'Libre Baskerville',serif;">
        [Heading: what the data shows]
      </h2>
      <p style="font-size:0.88rem; line-height:1.85; color:#a89f94;
         margin-bottom:0.7rem; font-family:'Libre Baskerville',serif;">
...
with col_chart:
    ...
    weekly = pd.read_csv("data/weekly_serbia.csv")
    ...
    fig2.add_annotation(
        x="2024-11-01", y=1, yref="paper",
        text="Nov 2024", showarrow=False,
    ...
    fig2.update_layout(
        ...
        xaxis=dict(
            tickformat="%b %Y", dtick="M12",
            gridcolor="rgba(255,255,255,0.05)",
            linecolor="rgba(255,255,255,0.1)", color="#666"
        ),
        yaxis=dict(
            title="Protests per week",
            gridcolor="rgba(255,255,255,0.05)",
...
st.markdown('<div style="border-top:1px solid #1e1e1e; margin:0;"></div>',
            unsafe_allow_html=True)
